...
OptPrefetcher::OptPrefetcher(const OptPrefetcherParams &p)
    : Queued(p),regionSize64(p.region_size_64),
          ...
          act_64("act_64", p.act_64_entries, p.act_64_entries, p.act_64_replacement_policy,
          p.act_64_indexing_policy, ACT64Entry()),
          opt("opt", p.opt_entries, p.opt_entries, p.opt_replacement_policy,
          p.opt_indexing_policy, OptEntry(2*(64-1), SatCounter8(3,0)))
{
}
void
    ...
    if (opt_entry) {
        ...
        for (int i = 0; i < OptLines - 1; i++) {
            if (opt_entry->hist[i + OptLines - 1].rawCounter() >= con_opt) {
                ...
                sendPFWithFilter(pfi, pf_tgt_addr, addresses, 1, PrefetchSourceType::SOpt, optPFLevel);
                                ...
                                PrefetchSourceType src, int ahead_level)
{
    if (filter->contains(addr)) {
        ...
        // NOTE: no pf req enqueue addresses?
        // if (ahead_level > 1) {
