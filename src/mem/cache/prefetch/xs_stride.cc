...
XSStridePrefetcher::XSStridePrefetcher(const XSStridePrefetcherParams &p)
    : Queued(p),useXsDepth(p.use_xs_depth),fuzzyStrideMatching(p.fuzzy_stride_matching),
      ...
      strideUnique("strideUniqueTable", p.stride_entries, p.stride_entries, p.stride_replacement_policy,
             p.stride_indexing_policy, StrideEntry()),
      strideRedundant("strideRedundantTable", p.stride_entries, p.stride_entries, p.stride_replacement_policy,
             p.stride_indexing_policy, StrideEntry()),
      nonStridePCs("nonStridePCsTable", p.non_stride_entries, p.non_stride_assoc, p.non_stride_replacement_policy,
             p.non_stride_indexing_policy, NonStrideEntry())
{
}
