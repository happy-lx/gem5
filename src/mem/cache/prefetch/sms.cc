...
XSCompositePrefetcher::XSCompositePrefetcher(const XSCompositePrefetcherParams &p)
    : Queued(p),
      ...
      act("act", p.act_entries, p.act_entries, p.act_replacement_policy,
          p.act_indexing_policy, ACTEntry(SatCounter8(2, 1))),
      re_act("re_act", p.re_act_entries, p.re_act_entries, p.re_act_replacement_policy,
          p.re_act_indexing_policy, ReACTEntry()),
      streamPFAhead(p.stream_pf_ahead),
      pht("pht", p.pht_entries, p.pht_assoc, p.pht_replacement_policy,
          p.pht_indexing_policy,
                                 ...
                                 PrefetchSourceType pf_source, bool miss_repeat)
{
    ...
    if (pf_source != PrefetchSourceType::SStream && !is_active_page) {
        ...
        if (use_pht) {
            ...
            phtLookup(pfi, addresses, late && pf_source == PrefetchSourceType::SPht, stride_pf_addr);
        }
...
void
XSCompositePrefetcher::notifyFill(const CacheAccessProbeArg &acc)
{
    const PacketPtr &pkt = acc.pkt;

    stats.refillNotifyCount++;
    berti->notifyFill(acc);
    if(pkt->req->hasVaddr())
