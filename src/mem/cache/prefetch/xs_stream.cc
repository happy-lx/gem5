...
XsStreamPrefetcher::XsStreamPrefetcher(const XsStreamPrefetcherParams &p)
    : Queued(p),
      ...
      stream_array("streamArray", p.xs_stream_entries, p.xs_stream_entries, p.xs_stream_replacement_policy,
                   p.xs_stream_indexing_policy, STREAMEntry()),
...
XsStreamPrefetcher::calculatePrefetch(const PrefetchInfo &pfi, std::vector<AddrPriority> &addresses, int late_num)
{
    ...
    streamLookup(pfi, in_active_page, decr);
